    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if client_form.is_valid() and image_form.is_valid():
                ...
                if img is None:
                    raise ValidationError("La imagen no se pudo cargar correctamente.")
                ...
                raise ValidationError("Los formularios no son válidos.")
        
        except Exception as e:
            # Log the exception and return an error message
            print(f"Error procesando la imagen o generando el PDF: {e}")
            return JsonResponse({'error': 'Hubo un error al procesar la imagen o generar el reporte PDF.'}, status=500)
...
def create_heatmap(image):
    ...
    try:
        ...
        raise ValidationError(f"Error al crear el heatmap: {e}")
...
def save_heatmap(heatmap):
    ...
    try:
        ...
        raise ValidationError(f"Error al guardar el heatmap: {e}")
...
def generate_pdf(request, client, image_upload, heatmap_url):
    ...
    try:
        ...
        raise ValidationError(f"Error al generar el PDF: {e}")
